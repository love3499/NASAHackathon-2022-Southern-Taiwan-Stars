...
define(['./Circle',
    ...
    function (Circle,
        ...
        var Control = function () {
            ...
            this.move_to_taiwan = function () {
                var coordinatesString = "23.633,120.931"; //taiwan 
                var Latitude = parseFloat((coordinatesString.split(",")[0]));
                var Longitude = parseFloat((coordinatesString.split(",")[1]));
                var coords = new WorldWind.Location(Latitude, Longitude, 10.0);
                this.goToAnimator.goTo(coords, 10.0);
            }
            window.ciot_controller = this;
            ...
            this.mostRecentSigEQ = function (GeoJSON) {
                ...
                for (var i = 0; i < GeoJSON.features.length; i++) {
                    if (GeoJSON.features[i].properties.mag > -1) {
            ...
            this.tourMetadataDisplay = function (array, index) {
                metadata.setName(array[index].properties.name);

                metadata.setEarthquake(array[index].properties.earthquake);
                metadata.setPrecipitation(array[index].properties.rainfall);
                metadata.setMudslide(array[index].properties.mudslide);
        ...
        window.ciot = new CIoT();
        myControl.redraw();



        // $.ajax({
        //     url: "http://nasahackthon.thebestyea.net:8092/get_points",
        //     method: "GET"
        // }).done((data) => {
        //     var points = data['data'];
        //     points.forEach(point => {
        //         window.ciot.Add_Point(point.level, point.lon, point.lat);
        //     });
        //     // Layer Manager
        //     //var layerManger = new LayerManager(wwd);
        //     myControl.redraw();
        //     setTimeout(function () {
        //         myControl.move_to_taiwan();
        //     }, 2500);
