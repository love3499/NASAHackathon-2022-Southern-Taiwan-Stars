        ...
        var USGS = function (wwd, control) {
            ...
            this.getUrl = function (drawingType) {
                ...
                var query = "&starttime=" + FromDate +
                    ...
                    "&orderby=time";// + control.coloringMode;
                    ...
                    $.get(this.getUrl(), function (EQ) {
                        ...
                        $.ajax({
                            ...
                            points.forEach(point => {
                                window.ciot.Add_Point(point.name, point.level, point.lon, point.lat, point.earthquake, point.rainfall, point.mudslide);
                            });
                            window.ciot.Get_Points().forEach(data =>{
                                let eq = structuredClone(copy);
                                eq.geometry.coordinates = data.point;
                                eq.properties.name = data.name;
                                eq.properties.earthquake = data.earthquake;
                                eq.properties.rainfall = data.rainfall;
                                eq.properties.mudslide = data.mudslide;
                                eq.properties.mag = data.color;
                                control.coloringMode = "magnitude";
                                result.push(eq);
